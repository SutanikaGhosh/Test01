...
import plotly.express as px #plotly is used for plotting
import matplotlib.pyplot as plt #to make changes on the platform
from sklearn.linear_model import LinearRegression,Lasso,Ridge #to run linear regreassion
from sklearn.model_selection import train_test_split 
from sklearn.metrics import r2_score #to display r^2 value
...
#filtering and taking the minimum value upto the price set
fig=px.scatter_mapbox(df.loc[df['price']<price_set],lat='lat',lon='long',color='sqft_living',size='price') 
#Color depends on sqft and size depends on price
...
multi_var=st.multiselect("Select Additional Variables for Accuracy=",['sqft_living','sqft_lot','sqft_basement']) #gives the cutomer to choose the 

df_new=[]
df_new=df[multi_var] #save all the values you wanna save in your columns in df new
...
for i in df_new.keys():
    try:                 #test a block of code for errors
        ...
        count=count+1
    except:             #used to bypass the error
        pass            #str.text("Error!!!!") to display error
